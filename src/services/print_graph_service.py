...
GRAPH_PNG_FILENAME = "graph_diagram.png"
GRAPH_PNG_PATH = os.path.join(tempfile.gettempdir(), GRAPH_PNG_FILENAME)
...
def generate_graph_png() -> str:
    """
    Generate the PNG for the graph and store it in temp.
...
def get_graph_png_path() -> str:
    """
    Return the PNG path, generate on demand if missing.
    """
    if not os.path.exists(GRAPH_PNG_PATH):
        return generate_graph_png()
    return GRAPH_PNG_PATH
