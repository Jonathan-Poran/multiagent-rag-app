...
sys.modules['server.graph.graph'] = MagicMock()

# Ensure server.graph remains a package
try:
    import server.graph
except Exception:
    pass  # server.graph exists; its 'graph' submodule is mocked

# Mock chains module before importing the node
mock_chains = MagicMock()
sys.modules['server.graph.chains'] = mock_chains
sys.modules['server.graph.chains.chains'] = mock_chains

# Now import the topic extraction node safely
from server.graph.nodes.topic_extraction_node import topic_extraction_node
